...
func (d *Service) RegisterProtocols(srv *rpc.Server, xp rpc.Transporter, connID libkb.ConnectionID, logReg *logRegister, g *libkb.GlobalContext) (shutdowners []Shutdowner, err error) {
	protocols := []rpc.Protocol{
		keybase1.AccountProtocol(NewAccountRPCHandler(xp, g)),
		keybase1.BTCProtocol(NewBTCRPCHandler(xp, g)),
		keybase1.ConfigProtocol(NewConfigRPCHandler(connID, g, d)),
		...
		keybase1.DeviceProtocol(NewDeviceRPCHandler(xp, g)),
		...
		keybase1.LogProtocol(NewLogRPCHandler(xp, logReg, g)),
		keybase1.LoginProtocol(NewLoginRPCHandler(xp, g)),
		keybase1.NotifyCtlProtocol(NewNotifyCtlRPCHandler(xp, connID, g)),
		keybase1.PGPProtocol(NewPGPRPCHandler(xp, g)),
		keybase1.RevokeProtocol(NewRevokeRPCHandler(xp, g)),
		keybase1.ProveProtocol(NewProveRPCHandler(xp, g)),
		keybase1.SaltpackProtocol(NewSaltpackRPCHandler(xp, g)),
		keybase1.SecretKeysProtocol(NewSecretKeysRPCHandler(xp, g)),
		keybase1.SessionProtocol(NewSessionRPCHandler(xp, g)),
		keybase1.SignupProtocol(NewSignupRPCHandler(xp, g)),
		keybase1.SigsProtocol(NewSigsRPCHandler(xp, g)),
		keybase1.TestProtocol(NewTestRPCHandler(xp, g)),
		keybase1.TrackProtocol(NewTrackRPCHandler(xp, g)),
		keybase1.UserProtocol(NewUserRPCHandler(xp, g)),
		keybase1.ApiserverProtocol(NewAPIServerHandler(xp, g)),
		keybase1.PaperprovisionProtocol(NewPaperProvisionRPCHandler(xp, g)),
