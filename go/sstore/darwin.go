...
type SecretStoreContext interface {
	GetAllUserNames() (libkb.NormalizedUsername, []libkb.NormalizedUsername, error)
...
func (k KeychainSecretStore) StoreSecret(accountName libkb.NormalizedUsername, secret []byte) (err error) {
...
func (k KeychainSecretStore) RetrieveSecret(accountName libkb.NormalizedUsername) ([]byte, error) {
	encodedSecret, err := keychain.GetGenericPassword(k.serviceName(), string(accountName), "", "")
	if err != nil {
		return nil, err
	}
	if encodedSecret == nil {
		return nil, libkb.SecretStoreError{Msg: "No secret for " + string(accountName)}
	}
...
func (k KeychainSecretStore) ClearSecret(accountName libkb.NormalizedUsername) error {
